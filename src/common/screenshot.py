...
def ss_full_page(ticker: str, url: str, output_dir: str, pipeline: str):
    path = output_path(output_dir, ticker)
    asyncio.run(_screenshot_full_page(random_proxy(), url, path, pipeline, ticker))


def ss_common_ancestor(ticker: str, url: str, texts, output_dir: str, pipeline: str):
    path = output_path(output_dir, ticker)
...
async def _screenshot_full_page(proxy: str, url: str, path: str, pipeline: str, ticker: str):
    ...
    try:
        ...
        log(error_name(e), pipeline, ticker)


async def _screenshot_common_ancestor(proxy: str, url: str, texts: list, path: str, pipeline: str, ticker: str):
    print(f'taking screenshot, url: {url}, path: {path}, proxy: {proxy}')
    try:
